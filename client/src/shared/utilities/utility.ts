...
export const Utility = {
  ...
  hasAccess: () => { return Utility.getToken() && Utility.getUser(); },
  ...
  isconfirmPasswordInValid: (confirmpassword: string | any[]) => {
    return confirmpassword.length < 8 && confirmpassword.length > 0;
  },
  /**
  ...
  updateImageUrl: (result: any[]) => {
    return result.map((item) => {
      ...
      if (!item.hasOwnProperty("imageUrl")) {
        ...
        if (imageSrc.indexOf("https:") < 0) {
          url = AppConstants.amazonNoImage;
        } else {
          if (imageSrc.indexOf(",") >= 0) {
            imageSrc = imageSrc              .split(",")
              .filter((item: string) => item.indexOf("_SS40_") >= 0);
            imageSrc = decodeURIComponent(imageSrc).trim();
          }
        }
        if (imageSrc.indexOf("._SS40_.") >= 0) {
          url = imageSrc.replace(/._SS40_./gi, "._SS500_.");
        }
      }
      item.imageUrl = url;
      return item;
