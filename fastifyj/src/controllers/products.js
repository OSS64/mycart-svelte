...
export const getProducts = async (req, reply) => {
	try {
		const result = await Products.limitedColumnsWithPaging(req.query.perPage, req.query.offSet, req.query.category);
		reply.code(200).send({ prodcutsCount: result.productsCount, products: result.products });
	} catch (err) {
...
export const getCategoriesList = async (req, reply) => {
	try {
		const categories = await Products.distinct('category');
		
		if (categories.length > 0) {
			reply.code(200).send({ status: true, statusCode: 200, categories: categories });
		} else {
			reply.code(200).send({ status: true, statusCode: 200, message: "No Category Found" });
		}
