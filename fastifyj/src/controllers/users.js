...
export const registerUser = async (req, reply) => {
    ...
    if(existingUserCheck){
        reply.status(200).send({ status:true, statusCode: 200, message: "User Already Registered" });
        return;
    }
...
export const userLogin = async (req, reply) => {
	try {
        if (!req.body.email && req.body.password) {
            reply.code(200).send({status: false, statusCode: 200, message: "Email is Required!"});
        } else if (!req.body.password && req.body.email) {
            reply.code(200).send({status: false, statusCode: 200, message: "Password is Required!"});
        } else if (req.body.email == "" && !req.body.password == "") {
            reply.code(200).send({status: false, statusCode: 200, message: "Email and Password are Required!"});
        } else if (!req.body.email && !req.body.password) {
